...
static unsigned char FILTER_APDU_0[] = { 0x00, 0xa4, 0x00, 0x0c, 0x02, 0x50, 0x31 };
static unsigned char FILTER_APDU_1[] = { 0x00, 0xa4, 0x04, 0x00, 0x0c, 0xa0, 0x00, 0x00, 0x00, 0x63, 0x50, 0x4b, 0x43, 0x53, 0x2d, 0x31, 0x35, 0x00 };
static unsigned char FILTER_APDU_2[] = { 0x80, 0xf6, 0x00, 0x01, 0x0a };
static unsigned char FILTER_APDU_3[] = { 0x80, 0xf6, 0x00, 0x00, 0x08 };
static unsigned char FILTER_APDU_4[] = { 0x00, 0xa4, 0x00, 0x0c, 0x02, 0x3f, 0x00 };

static int FILTER_APDU_LENGTHS[] = { 7, 18, 5, 5, 7 };

static unsigned char RESPONSE_OK[] = { 0x90, 0x00 };

/*This table maps the outgoing debit machine message codes strings to the corresponding
 *  internal enumerated type
 */
debitMachineTransactionTableEntry outgoing_table [OUT_CODE_MAX] =
{
...
//		//emit message(QString("Ignoring certain APDUs"));
	...
	qDebug() << "XXXX NfcWorker::prepareToEmulateEcho finished ...";
}
...
void NfcWorker::processIso144434EchoCommandEvent(nfc_target_t *target) {
	...
	QString text = QString::fromUtf8(commandData.constData(), command_length);
	//emit message(QString("Command APDU : %1").arg(commandAsHex));
	//emit message(QString("Text received : %1").arg(text));
...
debitMachineMessageType NfcWorker::debitMachineListener(nfc_target_t *target){
	...
	qDebug() << "Received Iso14443-4 Command message";
	nfc_result_t result = nfc_get_iso14443_4_emulation_command(target, command, NFC_ISO14443_4_COMMAND_BUFFER_LENGTH, &command_length);
	...
	int count = 0;
	const char * messageChar = messageString.toUtf8().constData();
	char codeArray[strlen(messageChar)];
	char textArray[strlen(messageChar)];
	char dataArray[strlen(messageChar)];
	int destIndex = 0;
	...
	for(int i=0; i<strlen(messageChar); i++ ){
		if (messageChar[i]=='!'){
			...
			if (count == 0){
				codeArray[destIndex] = messageChar[i];
				destIndex++;
			}
			else if (count == 1){
				textArray[destIndex] = messageChar[i];
				destIndex++;
			}
			else if (count == 2){
				dataArray[destIndex] = messageChar[i];
				destIndex++;
			}
		}
	}

	received_message.transactionCode = atoi(codeArray);
...
void NfcWorker::handlePaymentRequest(nfc_target_t *target){
	...
	switch (message_received.transactionCode){
		...
		case IN_CODE_MESSAGE_ACK:
			break;

		case IN_CODE_PEER_READY_REQ:
			outCode = OUT_CODE_PEER_DEVICE_READY_RESP;
			break;

		case IN_CODE_TRANSACTION_ID_RECEIVED_CONF:
			break;

		case IN_CODE_TRY_AGAIN_REQ:
			break;

		case IN_CODE_READY_FOR_RECEIPT_RESP:
			break;

		case IN_CODE_RECEIPT_VALID_MESSAGE:
			break;

		case IN_CODE_RECEIPT_INVALID_MESSAGE:
			break;

		case IN_CODE_READY_FOR_BALANCE_RESP:
			break;

		case IN_CODE_BALANCE_RECEIVED_CONF:
			break;
